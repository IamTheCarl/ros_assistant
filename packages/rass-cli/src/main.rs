...
fn application(args: arguments::RosAssistant) -> Result<()> {
    ...
    match args.subcommand {
        ...
        arguments::SubCommand::Deploy(deploy_args) => {
            deploy(args.build_machine, deploy_args).context("Failed to deploy project")
        }
        arguments::SubCommand::Ssh(ssh_args) => ssh(ssh_args).context("Failed to ssh to host"),
    }
}

fn ssh(args: arguments::SshCommand) -> Result<()> {
    let (_project_root, ssh_config) = load_project(args.project_root)?;

    let host = if let Some(host) = args.host {
        host
    } else {
        // TODO if there's only one host on the robot, just assume it's that one.
        bail!("Auto host-selection is not yet supported.");
    };
...
fn deploy(build_machines: Vec<String>, args: arguments::Deploy) -> Result<()> {
