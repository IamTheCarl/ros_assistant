...
use r2r::{
    create_bridge_interface::msg::{DriveArc, LEDState, SensorQuery},
    ...
    W: tokio::io::AsyncWrite + std::marker::Unpin,
{
    ...
    let mut drive_arc_left =
        node.subscribe::<DriveArc>("/drive/arc_left", QosProfile::default())?;
    let mut drive_arc_right =
        node.subscribe::<DriveArc>("/drive/arc_right", QosProfile::default())?;
    let mut drive_stop =
        node.subscribe::<r2r::std_msgs::msg::Empty>("/drive/arc", QosProfile::default())?;
